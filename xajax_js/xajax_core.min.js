...
xajax.config.cursor={update:function(){return{onWaiting:function(){if(xajax.config.baseDocument.body)xajax.config.baseDocument.body.style.cursor="wait"},onComplete:function(){xajax.config.baseDocument.body.style.cursor="auto"}}},dontUpdate:function(){return{onWaiting:function(){},onComplete:function(){}}}};xajax.tools={};
xajax.tools.$=function(sId){if(!sId)return null;if(typeof sId!=="string")return sId;var oDoc=xajax.config.baseDocument;var obj=oDoc.getElementById(sId);if(obj)return obj;if(oDoc.all)return oDoc.all[sId];return obj};xajax.tools.in_array=function(array,valueToCheck){var i=0;var l=array.length;while(i<l){if(array[i]==valueToCheck)return true;++i}return false};xajax.tools.doubleQuotes=function(haystack){if(typeof haystack=="undefined")return false;return haystack.replace(new RegExp("'","g"),'"')};
...
xajax.tools._nodeToObject=function(node){if(null==node)return"";if("undefined"!==typeof node.nodeName)if("#cdata-section"===node.nodeName||"#text"===node.nodeName){var data="";do if(node.data)data+=node.data;while(node=node.nextSibling);return xajax.tools._enforceDataType(data)}else if("xjxobj"===node.nodeName){var key=null;var value=null;var data=new Array;var child=node.firstChild;while(child){if("e"===child.nodeName){var grandChild=child.firstChild;while(grandChild){if("k"===grandChild.nodeName)key=
xajax.tools._enforceDataType(grandChild.firstChild.data);else if("v"===grandChild.nodeName)value=xajax.tools._nodeToObject(grandChild.firstChild);grandChild=grandChild.nextSibling}if(null!=key){data[key]=value;key=value=null}}child=child.nextSibling}return data}throw{code:10001,data:node.nodeName};};
...
xajax.js.setFunction=function(args){args.fullName="setFunction";var code=new Array;code.push(args.func);code.push(" = function(");if("object"==typeof args.prop){var separator="";for(var m in args.prop){code.push(separator);code.push(args.prop[m]);separator=","}}else code.push(args.prop);code.push(") { ");code.push(args.data);code.push(" }");args.context.xajaxDelegateCall=function(){eval(code.join(""))};args.context.xajaxDelegateCall();return true};
xajax.js.wrapFunction=function(args){args.fullName="wrapFunction";var code=new Array;code.push(args.func);code.push(" = xajax.js.makeWrapper(");code.push(args.func);code.push(", args.prop, args.data, args.type, args.context);");args.context.xajaxDelegateCall=function(){eval(code.join(""))};args.context.xajaxDelegateCall();return true};
...
xajax.attr={has:function(ele,attrib){var hasAttrib=false;try{hasAttrib=xajax.tools.$(ele).hasAttribute(attrib)}catch(error){throw error;}return hasAttrib},add:function(ele,attrib,value){if(!xajax.attr.has(ele,attrib)){ele=xajax.tools.$(ele);try{console.log(ele,value);value=value&&typeof value==="string"?value:"";ele.setAttribute(attrib,value)}catch(error){throw error;}return true}},remove:function(ele,attrib){if(xajax.attr.has(ele,attrib)){ele=xajax.tools.$(ele);try{ele.removeAttribute(attrib)}catch(error){throw error;
}return true}},replace:function(ele,attrib,value){if(xajax.attr.has(ele,attrib)){xajax.attr.remove(ele,attrib);return xajax.attr.add(ele,attrib,value)}else return xajax.attr.add(ele,attrib,value)}};xajax.dom={};
